  ...
  async saveRisingStarEntry(entry: RisingStarEntry): Promise<void> {
    try {
      ...
      await this.dbClient.execute(query, [
        ...
        entry.genre,
        entry.position,
        entry.captured_at ? new Date(new Date(entry.captured_at).getTime() + (7 * 60 * 60 * 1000)).toISOString().slice(0, 19).replace('T', ' ') : new Date(Date.now() + (7 * 60 * 60 * 1000)).toISOString().slice(0, 19).replace('T', ' ')
      ]);
